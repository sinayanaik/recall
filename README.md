...
- [Cloud bucket — papers and images in your own S3 bucket](#cloud-bucket--papers-and-images-in-your-own-s3-bucket)
...
| Signed out on one device after signing out on another | It used to be one sign-out for every device: `signOut()` defaults to a GLOBAL scope and revokes every refresh token the account has anywhere. The button says "Sign out", on one device, beside that device's decks — so it signs out `scope: "local"` now | Nothing to do. A sign-out ends the session on the device it was pressed on |
...
**The `documents` bucket is now read-only**, and holds only the PDFs uploaded before Recall moved PDF hosting out of Supabase. It keeps owner-scoped **read** and **delete** policies — read is what keeps those decks opening, delete is what lets **☰ → Cloud bucket** move them across — but it no longer has an INSERT policy, so nothing can add to it. Re-running this file drops the old upload policy. A project that never imported a PDF simply has an empty bucket, and can ignore it entirely.
...
## Cloud bucket — papers and images in your own S3 bucket

PDFs do not go to Supabase, and neither do new images. They go to **an S3-compatible bucket you supply** —
...
Everything about it lives in its own panel, **☰ → Cloud bucket**, which loads and
works on its own — nothing there waits for the counting in **Storage & Data**.
...
3. **Allow this site to reach the bucket.** In Recall, open
   **☰ → Cloud bucket** and press **Copy the CORS policy** — that
...
Recall files papers under a `recall/` prefix and images under `recall-images/`,
so a bucket you already use for something else is fine.
...
read path is still there for exactly this reason. Three things to know.
...
**Then move the papers across.** **☰ → Cloud bucket → Papers still elsewhere**
lists everything still in Drive or in the old Supabase bucket, with
**Move them to the bucket**. For each paper, in this order:

1. The bytes are read — from this device when it has them — and **hashed**. A copy
   that is not the version the deck uses is refused, and nothing changes.
2. They go into the bucket, and the bucket's copy is **checked**, byte count and all.
3. The deck records where the paper went, and your decks **sync**.
4. Only when the deck's row **in the cloud** carries that record — so every other
   device can find the paper in the bucket — is the old copy removed. A Supabase
   copy is removed only if it is the same size as the file moved, and only when no
   other deck, on this device or in the cloud, still relies on it; the last deck to
   move frees it.

A paper that got as far as step 3 but not step 4 is shown as *waiting*: it is
already safe in the bucket, nothing has been deleted, and pressing the button
again finishes it. The panel lists every paper that did not finish, and why. Once
a paper is done it is marked done on its record (`retiredLocators`), so it is not
offered again — which is what used to keep the card from ever emptying.

**Drive copies are left in your Drive.** Google no longer lets Recall delete
there, so once a paper is safely in the bucket its Drive file is left where it is,
the paper stops being listed as elsewhere, and the panel names it so you can
delete it at drive.google.com if you want the space. Reading a paper *out of*
Drive needs a device that can still reach it, so a paper whose only copy is in
Drive moves from a device that already holds the file.
...
Once the bucket is set up, **new images go into it** — pasted, dropped, picked,
drawn, or brought in by an EPUB. If the bucket turns an upload away (a CORS policy
missing, a read-only token, a full bucket) the image goes to Supabase instead, so a
misconfigured bucket never costs you a picture.

**Your notes do not change.** Every image keeps the address it always had — the
Supabase URL in the note's markdown is an identifier, not a download link, and has
been since the images bucket went private. The bucket stores each image under a key
built from that identifier (`recall-images/<your project>/<the same path>`), and
Recall decides per image, just before it loads, which storage holds it. Nothing in
any deck is rewritten, the offline copy of each image stays where it was, and a
backup, an export or a broken-image scan finds every image wherever it lives.

**Moving the ones already in Supabase** is two separate steps in
**☰ → Cloud bucket → Images**:

1. **Move images to the bucket** copies every image that is only in Supabase, and
   checks each one in the bucket. It deletes nothing, anywhere, and can be run as
   often as you like.
2. **Remove the Supabase copies** is what frees the Supabase space, and it asks you
   to type DELETE first. At the moment it runs, it reads both storages again and
   deletes only the images the bucket holds **at the same size** — and only ones you
   were shown and confirmed. It keeps this device's offline copies.

Before step 2, take a backup, and open Recall once on each of your other devices so
they are on this version: an older version only knows how to load images from
Supabase, so an image whose Supabase copy is gone shows as broken there until it
updates. Nothing is lost either way — the bytes are in your bucket.

Images you delete from a note are removed from Supabase as before, but a copy in the
bucket is kept, because another deck — or an undo — may still point at it.
...
| The bucket is full | The upload fails and says so. **☰ → Cloud bucket** shows what Recall is using; your provider's dashboard has the allowance. |
| An image, and the bucket is not set up or refuses it | It goes to Supabase, as before. Moving it across later is one button. |
| An image, offline | The offline copy shows it, whichever storage holds it. |
...
| `src/` | All application logic, as ES modules — 204 files. `src/main.js` is the entry point the page loads and holds no logic of its own; everything else is imported from it |
| `styles/` | All styling, including the 10 themes — 54 files, **loaded in numeric order** |
...
```sh
node tools/check.mjs           # everything except the release cycle
node tools/check.mjs --quick   # static checks only, no browser (~30s)
node tools/check.mjs --full    # ...and drive a real install / offline / update
node tools/check.mjs --list    # the names, for --only
node tools/check.mjs --only highlight
```

**This also runs in CI**, on every push and every pull request
(`.github/workflows/check.yml`). It did not use to, and that is most of why the
suite was in the state it was: css-parity spent months red with nobody reading
it, and twelve checks reported success having never started a browser.

Three things a check must now do, none of which it had to before:

- **end with a tally** — `47 checks · 0 failed`. No tally is a failure, whatever
  the exit code says, because "printed no tally" is what BOTH silent failures
  look like from outside: a check that skipped, and a check that died half way
  through with its remaining assertions unrun.
- **assert something** — a tally of zero fails too.
- **not skip.** `SKIP_BUDGET` is pinned at 0. A check that cannot run on this
  machine is a red line with its name on it, not an `ok`. The browser comes
  from `tools/browser.mjs`, which speaks the DevTools protocol directly and
  needs nothing installed.

Two failures are pinned rather than fixed, both in `tools/check.mjs` with the
evidence written out: `SELECTION_EXPECTED_FAILURES` (a mouse drag on a card
face selects nothing, cause not isolated) and `PORT_SYNC_EXPECTED_DRIFT` (two
...
| Check | Question |
|---|---|
| `split-parity` | Is any symbol the pre-split build had now nowhere in the tree, and does any name belong to two modules at once? (It used to diff every function BODY too. That stopped being a test the moment the move finished and the app carried on being developed — a changed body means somebody edited a function — and keeping it quiet cost a paragraph of prose per commit) |
...
| `s3-store` | Does a paper still come back now that it lives in your bucket — and does the move out of Drive and Supabase finish? A moved paper must never be offered again; an old copy is deleted only once the bucket copy is checked, the cloud's copy of the deck carries the hash every device needs, and no other deck still relies on it |
| `s3-images` | Do images reach your bucket and come back out of it without a note changing? The identifier-to-key mapping and the service worker's rebuild of it, where a new image goes when the bucket refuses, which storage an image is loaded from, and a clean-up that deletes a Supabase copy only when the bucket holds it at the same size |
...
| `ink` | Does handwriting survive being stored, and does the pen refuse what it should? A stroke is quantised to an eighth of a point, delta-encoded, packed into base64url and simplified before it is stored — every one of those loses something on purpose, and what must not be lost is the handwriting. So: a round trip to within one quantisation step, a one-sample stroke still being a dot (a full stop in a margin note), a version this build does not know decoding to *nothing* rather than to a best guess, and seven shapes of corrupted string refused rather than half-read. Then the question a book's sync once turned on: an encoded page is checked in the JSON **body**, not as a JavaScript string, for a `\u0000` escape or half a surrogate pair — the format that ships handwriting into `meta` is a format that could do to a paper what pdf.js's UTF-16 decoding once did to a title. Plus a measured size ceiling, so the encoding cannot quietly regress into re-uploading a megabyte per sync. The other half is the straightener, which is nothing but thresholds, and where the assertions that matter are the REFUSALS: a written word, a scribble, a tick and — the one the size floor was actually moved for — a letter O, which fitted an ellipse at the first threshold tried. Turning somebody's handwriting into a circle is far worse than never offering, so a deliberate ring round one symbol in an equation is asserted as still snapping, on the other side of the same number. Pure Node: the wire format, the shape fitter and the prediction bound are all leaves, deliberately, so this needs neither a browser nor the `pre-modular` tag. Then the browser's own guess: `getPredictedEvents` hands the engine a few samples ahead of the nib, and drawing all of them is the reported "tail of the strokes staying unnecessarily" — the predictor carrying straight on through the end of a letter, round the corner at the bottom of an `n`, or inventing motion for a nib that has stopped. `src/render/ink-predict.js` decides how much of a guess is worth drawing (a two-frame horizon, a reach capped against the step the hand actually just made, and a turn it may not exceed), and it is asked here because nowhere else can ask: headless Chrome does not synthesise predicted events for a dispatched `PointerEvent`, so a browser check cannot reach that code at all. A pure function over numbers in model units is the only instrument there is |
| `handwriting` | Does the pen keep writing when the hand pauses, is the ink it leaves actually VISIBLE, and does a notebook keep what was written on it? `ink` above is pure Node and can ask whether a stroke survives being stored; it cannot ask whether the line follows the nib. So this drives a real stylus through Chrome over a real generated PDF. **A stroke held still for 750ms and then continued** — the straightener fires on a hold, and a hold is usually somebody thinking; once it had fired every later sample was discarded. **The low-latency pair coming off the page once the stroke is committed** — reading the dry canvas's bitmap proves the ink was PAINTED and cannot prove it is visible, and those came apart: `desynchronized` asks to be taken out of the normal compositing path, and on Chrome/Android that can be a hardware overlay plane that does not blend with what is under it, so a cleared but still-mounted wet canvas hid the committed ink. **The 185th stroke costing what the 3rd did.** Then the notebook: a page added rewrites the document and not just the record of it, the pen writes on a page that did not exist a moment ago, and tearing one out renumbers every mark after the gap — quads included, since those are what a paint and a Go-to resolve against — while burying what was on the page itself. A markdown block dragged and resized in the page's own POINTS, with the y assertions spelled out because PDF y runs up the page and the screen's runs down. And the drawing colours, asked with the OS emulated to light and the app on a dark theme, which is the ordinary case and the one that made every SVG render black. Then the three the panel was rebuilt for. **A deck that already has a PDF being offered a notebook rather than a refusal** — two documents on one deck, with their own page counts, and a stroke made on one never appearing on the other, which is what one shared array of marks does the moment its `doc` field is not honoured. **Ink read off the canvas's own pixels across a theme change**: a stroke stores a pen token and the token resolves per theme, so this was never a storage fault — but a canvas is a bitmap, nothing cleared the resolved colour and nothing repainted, and "I wrote in white on a dark theme and it vanished on a light one" was exactly true. The paper under it is asked in the same breath, because a notebook page is a white PDF page and a near-white pen on one is invisible before any theme is switched at all. **And a picture on a page**: sized from its own aspect ratio rather than a paragraph's, dragged out by its grip, and coming back out of the store as a picture on the notebook's pages rather than as markdown. Then the four the panel was reported for a second time, which are all the same kind of fault — a control that exists and cannot be reached. **The pen's rail has a box on the page** on a deck with no PDF, where `.has-no-document` was hiding it while ✎ armed the pen and lit its own button: no colour, no nib, no eraser, no lasso, and no way for a mouse to draw at all. It is open on arrival and closing it is remembered. **Dark page is not offered** on paper that follows the theme, and `togglePdfInvert` refuses there as well, because a hidden button is not a removed mode. **A lasso selects, and offers what can be done with the selection** — all of which already worked and none of which anybody could get to. And **the grid is squared paper**, read off a scanline of the rendered page: enough rules to be paper at all, a minor rule that is really there, and a heavier one every fourth square, which is what stops A4 at 63% on a phone reading as a grey wash. Then the four this surface was reported SLOW for, and they are counts rather than clocks so they mean the same thing on a shared runner as on a desk. **A pen lift does not measure every page in the document** and **neither does a scroll**, counted through `HTMLElement.prototype.offsetTop`: the page-geometry table was rebuilt whenever it was more than a frame old, which is to say on essentially every call, and it is reached from the scroll pass, the page observer, the printed notes and the pen's rail at every stroke — so both of the gestures reported slow were measuring the whole paper several times a second. **Telling the app about a stroke does not cost a pass per page on screen**, counted as re-reads of `state.notes`: every rendered page used to ask `annotatedDocumentHighlights()` for itself, which is a sort of every record plus a parse of the whole note, about four times a second while somebody writes. And **a page built stroke by stroke is the page a full repaint would paint**, compared as two bitmaps off the same canvas — the pen lift now ADDS the committed stroke to the dry canvas instead of clearing it and painting every stroke on the page again, and a canvas missing a stroke looks exactly like a canvas that was never told about it, so nothing but a pixel compare can catch that. Plus the leak that stopped the pen dead: **a stroke whose pointerup never arrives** — the capture taken away mid-stroke — must not refuse every stroke after it, **nor the rail's stroke options**, because an unreleased pointer capture retargets every later event for that id, and a mouse is always id 1, which is why "I cannot draw anything" and "I cannot choose any stroke options" were one bug. Then the thing the pen could not do AT ALL. `inkTakesPointer` opened with `if (event.pointerType === "pen") return true;` and the press it started said `setInkPenDown(true)`, which is the flag the selection controller reads to stand down — so on a paper a stylus could draw and could do nothing else: no highlight, no cloze, no copy, no phrase lifted out into a note, every one of which that surface has had all along and only a finger or a mouse could reach. `src/format/ink-colors.js` says in as many words that this app's highlighter is the one that marks the words you SELECTED; the stylus had no way to select any. So the rail has a fourth tool, and these are the two halves of one drag asked separately, because they are two separate failures: **the ink that must not appear** and **the selection that must**, on the page the nib was actually on and carrying the words it covered. Then that the selection is one the HIGHLIGHTER can act on, through the same pair the pill resolves to — which is the whole argument for the mode and not a second assertion of the first. That both doors say which way it is set and the colours and the nibs fold away, since "a control that exists and cannot be reached" is what this panel has been reported for twice. That **a tap is still a tap** under the slop, or a highlight's own menu could never be opened with the pen again. That the pen still **draws** once it is armed back, because a mode is not a removal. And that a stylus **turned over still rubs out** while Text is armed, which the eraser's own tooltip has promised all along. Then the half of that mode nobody had asked about: which SURFACE it belongs to. The PDF tab and the Write tab are one `#documentStage` showing the deck's two documents, and they were one ink engine with one tool between them — so Text armed on the paper walked onto the notebook still armed, and the stylus swept a selection across blank paper and drew nothing, with the tool row shut and nothing on screen to say why. So: **Text on the paper does not follow the pen onto the notebook**, **the first stroke there is a stroke**, and **the paper still has Text armed when the reader goes back to it**. The eraser and the lasso deliberately do NOT come back across a tab switch — the same rule `inkPreferences()` keeps for a new session, and for the same reason. Alongside it, the way out: `setInkArmed(false)` reached past `setInkTool` to the engine, writing the engine's tool and leaving both the flag the selection controller reads and the class the cursor is drawn from still saying "this stylus selects", so the ink layer started a stroke and touch-selection started a drag over it from one contact — **shutting the rail takes the stylus all the way out of Text**, and **the pen draws again with the rail shut**. And one line about paper: a new notebook is made on **the paper this device last wrote on** rather than the built-in grid, so choosing ruled is a choice you make once instead of on every deck you ever start |
...
| `sync/` | Two-way sync — the per-card merge, tombstones, push/pull, and the deletion guards. `document-sync.js` is the same four things for a paper's annotations: a per-record timestamp, a merge on the pull, a reconcile against the cloud's real row before the push, and a tombstone for a deleted highlight. The rule everything here rests on is written above `pushLibraryDeckToCloud`: **a push is a merge of every class of content, never a replace** — one deck-level `updatedAt` decides the direction, so a device never pushes AND pulls the same deck, and any device with a local change of any kind pushes. A push from a device that did not touch class *C* must leave the cloud's copy of *C* alone, for all twelve of them, and `tools/sync-reconcile-check.mjs` asserts that once per class so the next key added to the meta bag without a rule fails there rather than in somebody's library. The notes body was the one class where it was false: both conflict gates asked whether the DECK had moved, which is true when one ink stroke moved it, so two devices with a note open conflicted on every sync and the push sent an untouched older body over the other device's edit. `syncTextFingerprint` (in `diff.js`) is the body as the cloud last held it, in eight bytes on the index entry, and it is what lets the gates ask the question they meant. `notes-merge3.js` is what happens when both devices really did type: a line-based three-way merge against the body they last agreed on, so edits three paragraphs apart are applied together and only the same lines are a question. And the write underneath all of it is a compare-and-swap now — an UPDATE predicated on the `updated_at` the merge was computed against — because every merge is computed against a row read once, up front, and whatever landed in the gap used to win the whole column |
| `storage/` | The device's own copy: IndexedDB deck store, autosave, quota, the storage panel. `deck-tab.js` is which panel each deck was last left on, per device — a leaf importing nothing, because `documents/doc-slot.js` reads it and that module must not take part in a cycle |
| `library/` | My Decks: the local index, folders, rows, tiles, drag and drop, and reading a whole folder as one document |
| `render/` | Markdown → HTML: math, clozes, note links, diagrams, tables, and the block cache that keeps huge notes fast. Also the pen: `ink-engine.js` is the stroke engine every drawing surface is driven by (a canvas per host for finished ink, one shared pair over it for the stroke under the nib and the frames drawn ahead of it), `ink-paint.js` turns points into the filled outline that makes a line vary in width, `ink-shapes.js` decides — reluctantly — when a held stroke meant a circle, and `ink-predict.js` decides how much of the browser's guess at where the nib is going next is worth drawing (all of it is the tail people see hanging off the end of a letter). Past ~2,000 blocks a note is cut into spans at provably safe lexer boundaries and each span is lexed and built only as the reader comes near it, so opening a note costs a screenful rather than a book (`viewport` above is the proof) |
...
| `documents/` | The Document surface: a PDF rendered page by page by pdf.js, its highlights (text runs, dragged regions *and* pen strokes — all stored in PDF user space so they survive a zoom and a reload; `pdf-ink.js` is the pen, including the one rule that makes it work without a mode: a pen draws, a finger scrolls, and a press too short and too still to be either is a tap that passes through), its contents drawer — the file's own outline where it has one, and a contents read out of the type on its pages where it does not, cached on the deck — the blocks dropped onto its pages (`pdf-blocks.js`: markdown, or a picture, each a rectangle in the page's own points — dropped by `+ Text`, picked up by a press so that Delete, the arrows, Ctrl+D and `[`/`]` mean it, and undoable, which a tombstoned delete had no way to be. A block carries no header of its own any more — Style, Edit and Delete used to be three buttons drawn across the top of EVERY block, permanently, for controls that act on at most one at a time; picking one up now surfaces them in `#inkRailBlock` over the page (`ui/ink-rail.js`), the same place a lassoed stroke's own actions already live in Selection, and the whole block is the drag handle in the bar's place; what one LOOKS like is `block-style.js`: a fill and an ink as TOKENS resolved per theme, for the reason the pen's colours are; a size, a code size and a picture width as typed NUMBERS, in the page's own points, because `xs s m l xl` is five sizes and no sixth and because the app's own settings panel already writes down the rule — no sliders, no clamps, whatever you type is applied; a face out of the same thirty-two the Style panel offers (`ui/theme-catalog.js`, resolved and fetched by `ui/fonts.js`); an alignment, a frame, whether the height follows the text and whether code wraps. Written onto the record only when it is not the default, because that array is re-sent whole on every push — and remembered per device in `storage/ink-prefs.js` beside the pen's own colour, so the next block starts where the last one ended. The controls are one panel in two homes (`block-style-bar.js`, in the editor window and in a popover on the block): a Fill/Text switch over ONE row of chips, a typed size and three alignment icons, and everything else behind one `More` — which is the shape `style-schema.js` reached for the same reason, after the first version put seven rows of pills on screen at once), the notes printed under each page, and the device/cloud store the file itself lives in. A deck has TWO document slots, not one — `doc-slot.js` is the leaf that names them: `meta.pdf` is the paper somebody gave us and `meta.notebook` is paper this app wrote itself (`blank-pdf.js`; add/tear-out/paper are `notebook.js` regenerating that file, and every page shares one content stream, so sixty pages is under ten kilobytes and a rewrite is cheaper than an incremental edit). One shelf was what made the app say "this deck already has a PDF" to somebody who wanted to write beside one. The marks stay in one array each with a `doc` field saying which paper they are coordinates into, because `mergeRecordsById` carries fields it does not know about straight through — so one more field is free everywhere and a second array is free nowhere |
...
| `handwriting/` | The **Write** tab, which IS the Document surface rather than imitating it: the same `#documentStage`, in the same box the PDF tab uses, showing the deck's other document (`src/documents/doc-slot.js`). So what you write on is a real PDF, laid out by pdf.js, with the pen storing strokes in the document's own user space, and a deck can carry a paper to read AND blank pages to write on. Building a second surface here instead would have meant a second render loop, a second IntersectionObserver, a second zoom and a second coordinate system, each with every bug the first one has already had. It was a full-page overlay opened from the ☰ drawer until it was not, and the overlay cost three things: a surface scoped to one deck living in the menu that holds what is true of the app, a forced refit on every open, and — the reported one — a `desynchronized` ink canvas re-parented into a `position: fixed` box on every open, which is flicker on the one surface whose whole job is keeping up with a nib. Its controls are the Document surface's own furniture, over the page: `+ Text`, `+ Image` and `+ Page` are a group on the pen's rail, and the paper and the tear-out are rows of `⋯`. Those three say those words now, and did not: they were `+✎`, `+📷` and `+▯` — a plus jammed against a pencil, a full-colour camera emoji standing a head taller than every monochrome glyph beside it, and a rectangle that is tofu on any font without it — crammed two glyphs at a time into a 30px square. Clear was `⎧`, which is not a symbol for anything: it is the top hook of a curly brace. Copy and Duplicate were two squares nobody could tell apart. `styles/56-pen-text.css` had already written the rule down while solving the same problem for one button — there is no symbol for this that renders on Android, iOS and desktop alike — so the controls that are reached for constantly and ARE iconic (the pen, the eraser, the lasso, undo, redo) keep their glyphs, and everything a reader had to stop and decode carries the word instead, as a pill the height of its neighbours. Snap shapes moved out of the tool group with them: it is a switch that holds across every tool, it wore the same solid accent fill the armed tool wears, and a row answering "which ONE tool is armed" was lighting two buttons at once. They were briefly lifted into `#viewModeRow` beside the document's, and that row is a NOWRAP flex line with a budget — four tabs plus nine controls in 390px, and the tab labels came out clipped mid-word as `CAR NOT DOCU WRI`. A row does not get wider because you put more in it. What else is in this directory is the drawing SHEET's paper: `pages.js` is a leaf that knows what a page is, `paper.js` is the stack and the engine over it, `rail.js` is the pen's controls — which existed twice until it |

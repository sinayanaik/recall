...
| `handwriting` | Does the pen keep writing when the hand pauses, and does a notebook keep what was written on it? `ink` above is pure Node and can ask whether a stroke survives being STORED; it cannot ask whether the line follows the nib, and that is what was reported. So this drives a real stylus through Chrome and asks the three questions only a compositor and a clock can answer. **A stroke held still for 750ms and then continued** — the straightener fires on a hold, and a hold is very often somebody thinking; once it had fired every later sample was discarded and the fitted SHAPE was committed, so the line stopped following the nib and the writing after the pause was gone. **The finished stroke reaching the dry canvas before the wet layer gives it up**, read synchronously at the lift with no frame in between: the wet pair is `desynchronized` and may present ahead of the compositor, so clearing it first put a one-frame hole where the stroke had just been, at every pen lift. And **the 185th stroke on a page costing what the 3rd did**, because three separate things used to be O(everything on the page) per stroke and the symptom of all three was the same — writing got slower the more you had written. Then the notebook, which is a deck: pages added, torn out (the tombstone, not the absence) and read back from IndexedDB, a text box dragged, resized and still where it was put, and the same notebook edited on two devices and merged. The one it caught while being written is the one nothing else could have: page rects were cached at layout time and a stack of pages SCROLLS, so a stroke made after any scroll landed at the offset the page used to be at |
...
| `handwriting/` | Pages you can write on, and the notebook made of them. A notebook **is a deck** — its pages live in that deck's `meta.pages` — which is why there is no new table, no new bucket and no migration behind this: a deck already has a snapshot, a cloud row, a per-key meta merge, folders, search, `[[links]]`, an export and a backup entry, and a second entity would have to be given every one of them again. `pages.js` is a leaf that knows what a page IS (794x1123 model units, A4 at 96dpi, whatever it is shown at) and imports nothing but the stroke format, so a Node check can drive it. `paper.js` is the stack and the engine over it, shared by the notebook and the drawing sheet inside a note — it hands the engine a plain scale as its matrix, which is what makes the strokes stored the strokes drawn on every device and at every zoom, the same call `pdf-ink.js` makes when it stores in PDF user space. `rail.js` is the pen's controls, which existed twice until this. `text-boxes.js` is the markdown boxes dropped onto a page: their words live in `meta` and nowhere else, so they are **not** reachable from search or `[[links]]` — a deliberate trade, written at the top of that file, with the notebook's export as the thing that keeps a page from being write-only |

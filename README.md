...
| `handwriting/` | The **Handwritten Notes** panel, which HOLDS the Document surface rather than imitating it: `#documentStage` is moved into it while it is open and moved back when it closes — moved, never cloned, the same rule `liftNotesControlsIntoRow` follows and for the same reason. So what you write on is a real PDF, laid out by pdf.js, with the pen storing strokes in the document's own user space. A deck with no document of its own is given generated blank paper (`src/documents/blank-pdf.js`, and add/tear-out/paper are `src/documents/notebook.js` regenerating that file — every page shares one content stream, so sixty pages is under ten kilobytes and a rewrite is cheaper than an incremental edit). Building a second surface here instead would have meant a second render loop, a second IntersectionObserver, a second zoom and a second coordinate system, each with every bug the first one has already had. What is left in this directory is the drawing SHEET's paper: `pages.js` is a leaf that knows what a page is, `paper.js` is the stack and the engine over it, `rail.js` is the pen's controls — which existed twice until it |

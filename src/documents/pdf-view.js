...
// position is a PAGE, a ratio down it and a fraction across it rather than a
// pair of scroll offsets, for the reason meta.readingPosition is: a scroll
// offset is only meaningful at the scale it was taken at, and the surface may
// well be re-fitted to a different one before this is restored.
//
// The third of those is the newest and only exists past fit-width, where the
// page is wider than the scroller — which is where a reader of a two-column
// paper spends their time. See currentDocumentAcross.
//
...
function parkOpenDocument() {
  ...
  //
  // The column has the same rule and its own way of failing it: a detached page
  // reports offsetWidth 0, currentDocumentAcross's guard turns that into 0, and
  // 0 is a perfectly plausible column — the left edge of the page. So a read
  // moved below the remove() does not throw and does not look wrong; it silently
  // reinstates the very fault this ordering exists to prevent, with the fix
  ...
  // collapses its scrollWidth to its clientWidth, so the browser clamps
  // scrollLeft to 0 here — before the entry below is even written, and a whole
...
function renderMissingDocumentPrompt(pdfMeta, pdfId = null) {
  renderDocumentPickPrompt({
    ...
    // Four situations now, not two. "No bucket is set up" is a different
    // sentence from "the download failed" because the reader can actually do
    // something about the first one, and being told to check their connection
    // when the truth is that this install has no cloud for papers at all sends
    // them looking in the wrong place. The Drive line survives for a record
    // written before the move, on a device that never held the file — the
    // reader's answer there is the migration, not a re-attach.
...
// stored reading position, and { page, ratio, across } coming back from the
// park, which resumes where the reader actually was a moment ago — down the
// paper AND across it, which is the half a stored position does not carry.
//
...
function finishDocumentOpen(view, token, openSlot, at, { restored = false, refit = false } = {}) {
  ...
  try {
    ...
    //
    // Note that this reaches applyPdfPageNotes, which re-lands the reader itself
    // when a note block changed height — with a page and a ratio, from here, at a
    // moment when scrollTop is still 0. On a restore it almost always finds every
    // block's signature unchanged and returns; when it does not, the land below
    // corrects it and no frame is painted in between. It is the sharpest reason
    // an omitted `across` must leave scrollLeft alone: a note growing above the
    // reader moves them DOWN the page and must not move them across it.
  ...
  const landOnReadingPosition = () => {
    // `across` on the park branch only. A stored reading position has no column
    // in it — it is a page and a ratio, written for a surface that had no
...
// ...and the same question on the other axis, which only has an answer at all
// past fit-width: below it the page is narrower than the scroller and there is
// nothing to scroll sideways. The subtraction is valid for exactly the reason
// above — one offsetParent for both elements — and it is simpler here, because
// .document-scroll's padding is `16px 0 48px`: no horizontal padding at all, so
// this is straight scroll-x with no bias of its own to remove.
export function pageOffsetLeft(entryEl) {
  const view = el.documentView;
  if (!entryEl) return 0;
  return entryEl.offsetLeft - (view?.offsetLeft || 0);
}
...
//
// A reader zoomed past fit-width is looking at part of a page: on a two-column
// paper, usually one of the two columns. Their scrollLeft is the whole of what
// says WHICH, and every path that rebuilds this surface drops it — the switch
// between the two tabs emptied the scroller three times over (the park's
// host.remove(), the teardown's innerHTML and the restore's) and never wrote it
// back. So the zoom came back and the column did not, and a reader who left off
// in the right-hand column of page 4 was returned to the left-hand one.
//
// A fraction of the PAGE, exactly as `ratio` is, and for the same reason: a
// scroll offset is only meaningful at the scale it was taken at, and the pages
// may well be re-fitted to another one before this is handed back. It is also
// why this is not a fraction of the pannable range (scrollLeft / (scrollWidth -
// clientWidth)) — that denominator moves when the WINDOW changes with no zoom at
// all, so a device rotated while the reader was on the other tab would land them
// somewhere new, and at fit-width exactly it is 0/0.
export function currentDocumentAcross() {
  const view = el.documentView;
  if (!view || !openPdf) return 0;
  // scrollLeft is a scroll offset rather than a geometric one, so reading it
  // forces no layout — the same argument currentDocumentPage's memo makes about
  // scrollTop. Below fit-width it is pinned at 0 and there is no column to
  // remember, which is nearly every reader, and they pay one free read for it.
  if (view.scrollLeft <= 0) return 0;
  // ONE page's box, and deliberately not a `lefts`/`widths` pair on
  // documentPageGeometry: that table is built for every page of the document and
  // sits on the pen's and the scroll pass's hot paths (see the notes on it). By
  // the time this runs, currentDocumentPage has already flushed layout in this
  // same pass, so these are two property reads on clean layout and no new flush.
  const entry = openPdf.pages.get(currentDocumentPage());
  const width = entry?.el.offsetWidth;
  if (!width) return 0;
  // No clamp to 0..1, unlike the ratio above. That clamp is there because
  // scrollTop can land in the 16px GAP between two pages, where the fraction
  // into the page the scan names goes negative. There is no horizontal gap — the
  // pages are one column — so the only way this leaves the range is a document
  // whose pages are not all the same width (fitWidthScale fits page 1, so a
  // landscape page 3 overflows even on fit-width), and there the out-of-range
  // answer is the correct one. The browser clamps scrollLeft to the real extent
  // on the way back for free, which is also the right thing to happen if the
  // page no longer overflows when the reader returns.
  return (view.scrollLeft - pageOffsetLeft(entry.el)) / width;
}
...
export function scrollToDocumentPage(pageNumber, ratio = 0, { smooth = true, align = "top", span = 0, across = null } = {}) {
  const view = el.documentView;
  ...
  const to = { top: Math.max(0, top), behavior: smooth ? "smooth" : "auto" };
  // ── ...and sideways, but only when the caller has an opinion about it ──────
  //
  // `across` is currentDocumentAcross's answer, and only the park hands one in.
  // Every other caller of this — the pager, a contents row, a jump to a
  // highlight, the printed notes re-landing after a block changed height, the
  // resume on a cold open — means "this page, this far down" and says nothing
  // about the reader's column. Moving it for them would be a surface that pans
  // sideways whenever a note above grew a line.
  //
  // A key that is absent is how ScrollToOptions spells "leave that axis alone",
  // so the option is BUILT rather than written as `left: across ? … :
  // view.scrollLeft` — that spelling would turn a read into a forced layout on
  // every press of the pager. And it goes in the same scrollTo rather than a
  // scrollLeft assignment after it, so the two axes share a behaviour: a smooth
  // glide down with the column snapping instantly is worse than either alone.
  //
  // The offsetWidth read is deliberately before the scroll. On the refit path
  // this runs between relayoutDocument's two passes, where the pages carry their
  // new inline width but layout has not flushed — and reading forces the flush
